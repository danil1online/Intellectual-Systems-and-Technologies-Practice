...
php /var/www/html/occ config:system:set oidc_login_provider_url --value="http://keycloak:${KEYCLOAK_PORT}/auth/realms/istp" 2>/dev/null || true
php /var/www/html/occ config:system:set oidc_login_well_known_url --value="http://keycloak:${KEYCLOAK_PORT}/auth/realms/istp/.well-known/openid-configuration" 2>/dev/null || true
