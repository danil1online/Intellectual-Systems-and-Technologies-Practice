...
### Авторизоваться на сервере [Jupyter-Hub](https://jupyter.org/hub) 
...
1. Подготовьте отчёт в Markdown: `Pr_22_<группа>_<номер>.md` (или `reports/Pr_22/Pr_22_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные модели;
   - выбранные метрики качества классификации изображений;
   - расчёты, результаты и интерпретацию метрик;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_22_<группа>_<номер>.md
