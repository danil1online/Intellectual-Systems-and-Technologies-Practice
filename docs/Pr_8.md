# Практическая работа №8: Машинное обучение. K‑Means Clustering
...
📚 Кластеризация (или кластерный анализ) — это процесс группировки объектов или данных в подмножества (кластеры), при котором объекты внутри одного кластера максимально похожи друг на друга по определенным критериям, а объекты из разных кластеров — максимально отличаются. 
...
Создайте новый каталог `Pr_8`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_8_report.ipynb` (в `~/project/reports/Pr_8`)
...
1. Сохраните Jupyter Notebook `Pr_8_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_8` в `reports` -> `project`)
