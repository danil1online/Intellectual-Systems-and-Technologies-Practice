...
1. Подготовьте отчёт в Markdown: `Pr_24_<группа>_<номер>.md` (или `reports/Pr_24/Pr_24_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные модели;
   - выбранные метрики генерации изображений;
   - расчёты, наборы сгенерированных изображений и их интерпретацию;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_24_<группа>_<номер>.md
