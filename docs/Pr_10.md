# Практическая работа №10: Машинное обучение. Density-Based Clustering
...
- Попробуйте кластеризовать указанный выше набор данных методом [k-средних](Pr_10.md), не создавая данные заново
...
1. Сохраните Jupyter Notebook `Pr_10_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_10` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_10 notebook report" && git push
```
