# Практическая работа №11 Машинное обучение. Hierarchical Clustering
...
Создайте новый каталог `Pr_11`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_11_report.ipynb` (в `~/project/reports/Pr_11`)
...
1. 🧪 **Генерация набора случайных данных**
  - Задаем _центр отсчета_ для всех генерируемых случайных значений
    ...
    np.random.seed(0)
    ```
  - Создадим случайные кластеры точек с помощью класса [make_blobs](Pr_11.md#-k-средних-на-случайно-сгенерированном-наборе-данных).
    ```python
...
6. 🧪 **Кластеризация с использованием scikit-learn**
...
1. Сохраните Jupyter Notebook `Pr_11_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_11` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_11 notebook report" && git push
```
