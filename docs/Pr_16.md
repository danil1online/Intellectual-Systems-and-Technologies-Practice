# Практическая работа №16 Машинное обучение. Content-based filtering
...
Создайте новый каталог `Pr_16`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_16_report.ipynb` (в `~/project/reports/Pr_16`)
...
1. Сохраните Jupyter Notebook `Pr_16_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_16` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_16 notebook report" && git push
```
