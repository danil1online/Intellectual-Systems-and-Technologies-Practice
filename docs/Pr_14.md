# Практическая работа №14: Машинное обучение. Logistic Regression with Python
...
Создайте новый каталог `Pr_14`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_14_report.ipynb` (в `~/project/reports/Pr_14`)
...
1. Сохраните Jupyter Notebook `Pr_14_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_14` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_14 notebook report" && git push
```
