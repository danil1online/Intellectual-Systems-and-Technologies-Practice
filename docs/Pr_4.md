...
1. Подготовьте отчёт в Markdown: `Pr_4_<группа>_<номер>.md` (или `reports/Pr_4/Pr_4_<группа>_<номер>.md`).
2. В отчёте укажите:
   - структура репозитория и файлы: `bot.py`, `requirements.txt`, `Dockerfile`;
   - команду сборки образа: `docker build -t <image_name> .`;
   - команду запуска контейнера: `docker run --restart=always -d --name <container_name> <image_name>:<tag> -p 8000:80`;
   - вывод `docker ps` или подтверждение, что контейнер работает и бот доступен;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_4_<группа>_<номер>.md
...
4. Что делает флаг `--restart=always` при запуске Docker-контейнера?
5. Как проверить, что чат-бот запущен в контейнере и реально отвечает?
