# Практическая работа №23: Метрики оценки качества сегментации и детекции изображений. 4 часа  
...
**Данный пример выполняет сегментацию и размещает результаты в локальной папке `./output/PennFudanPed/`. Cначала нужно создать `run_inference.py` и запустить, потом создать `evaluate_metrics.py` (показан ниже) и запустить**
...
1. Генерация предсказанной маски. Скрипт `run_inference.py`
```python
#!/usr/bin/env python3
import os
import json
...
```
После этого в каталоге `./output/PennFudanPed/pred_masks` были созданы PNG-маски:
...
2. Вычисление метрик. На основе `mask_gt` и `mask_pred` рассчитываются:
...
Код примера, подготавливающего сегментацию `evaluate_metrics.py`:
