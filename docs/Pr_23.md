...
**Данный пример выполняет сегментацию и размещает результаты в локальной папке `./output/PennFudanPed/`. Cначала нужно создать `evaluate_metrics.py` (в нем объявлен класс `PennFudanDataset`), потом создать `run_inference.py` (показан ниже) и запустить (команда запуска тоже приведена ниже). Только после этого, запустить `evaluate_metrics.py` **
