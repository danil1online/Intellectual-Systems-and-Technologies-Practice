...
def main(data_root, batch_size):
    ...
    # Фиксируем сид, чтобы таблица метрик была воспроизводимой
...
- `torch.manual_seed(42)` — фиксирует случайную инициализацию необученных декодеров, поэтому таблица метрик одинакова при каждом запуске.
- Если бы у какой-то модели все логиты оказались ниже 0.5 (для необученного декодера это вполне вероятно), предсказанная маска была бы пустой и обе метрики дали бы ровно `0.0000` — это нормальный результат, а не ошибка данных.
...
**📌 Ожидаемый результат**
Таблица `Architecture | Mean IoU | Mean Dice` с тремя строками (`Unet-R34`, `FPN-R34`, `DeepLabV3`). Сид зафиксирован (`torch.manual_seed(42)`), поэтому числа воспроизводимы:

```
Architecture |  Mean IoU  | Mean Dice
---------------------------------------
Unet-R34     |   0.1613   |   0.2714
FPN-R34      |   0.2394   |   0.3777
DeepLabV3    |   0.1169   |   0.2038
```

Значения невысокие: декодеры не обучались на Penn-Fudan, поэтому метрики отражают уровень моделей «из коробки», а не качество дообученной модели. Без зафиксированного сида числа были бы другими при каждом запуске — вплоть до того, что вся строка оказалась бы ровно `0.0000`: при случайном декодере все логиты могут оказаться ниже порога 0.5, тогда предсказанная маска пуста, и IoU с Dice дают ноль.
