...
**Данный пример приведен для изучения. Он выполняет сегментацию и размещает результаты в локальной папке `./output/PennFudanPed/`. Одного выполнения (уже сделано) достаточно. Если есть желание его повторить, то сначала нужно создать `evaluate_metrics.py` (показан под основным текстом), а затем уже запускать предсказание**
...
```
Его запуск из рабочей папки студента:
