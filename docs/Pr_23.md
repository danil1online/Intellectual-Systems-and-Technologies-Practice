...
**Данный пример выполняет сегментацию и размещает результаты в локальной папке `./output/PennFudanPed/`. Cначала нужно создать `evaluate_metrics.py` и запустить, потом создать `run_inference.py` (показан ниже) и запустить**
...
1. Вычисление метрик. На основе `mask_gt` и `mask_pred` рассчитываются:
