...
0. Подключитесь к Jupyter-Hub-ИИСТ-НПИ из [предыдущей работы](Pr_22.md#%EF%B8%8F-настройка-среды)
...
1. Подготовьте отчёт в Markdown: `Pr_23_<группа>_<номер>.md` (или `reports/Pr_23/Pr_23_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные модели;
   - выбранные метрики сегментации и детекции;
   - расчёты, визуализацию результатов и их интерпретацию;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_23_<группа>_<номер>.md
