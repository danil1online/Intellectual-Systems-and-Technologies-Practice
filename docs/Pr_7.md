# Практическая работа №7: Работа с системами визуализации пространственных данных
...
Создайте новый каталог `Pr_7`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_7_report.ipynb` (в `~/project/reports/Pr_7`)
...
1. Добавьте в конец Jupyter Notebook `Pr_7_report.ipynb` Markdown-ячейку с ответами на контрольный вопрос из раздела [«Контрольные вопросы»](Pr_7.md#контрольные-вопросы)
...
3. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_7` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_7 notebook report" && git push
```
...
## Контрольный вопрос
Изменить вид карты и представить результаты в отчете, а также подробно рассказать о библиотеке Folium и ее аналогах.
