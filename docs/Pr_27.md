...
1. Подготовьте отчёт в Markdown: `Pr_27_<группа>_<номер>.md` (или `reports/Pr_27/Pr_27_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные LLM;
   - выбранные метрики генерации текста;
   - примеры входных промптов, сгенерированных текстов и расчёты метрик;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_27_<группа>_<номер>.md
