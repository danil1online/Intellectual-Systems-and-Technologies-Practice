# Практическая работа №13: Машинное обучение. SVM (Support Vector Machines)
...
Создайте новый каталог `Pr_13`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_13_report.ipynb` (в `~/project/reports/Pr_13`)
...
1. Сохраните Jupyter Notebook `Pr_13_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_13` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_13 notebook report" && git push
```
