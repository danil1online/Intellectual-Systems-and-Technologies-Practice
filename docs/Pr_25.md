...
1. Подготовьте отчёт в Markdown: `Pr_25_<группа>_<номер>.md` (или `reports/Pr_25/Pr_25_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные модели;
   - выбранные метрики распознавания речи;
   - расчёты, примеры аудио/транскриптов и их интерпретацию;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_25_<группа>_<номер>.md
