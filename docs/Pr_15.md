# Практическая работа №15: Машинное обучение. Collaborative filtering
...
Создайте новый каталог `Pr_15`

Создайте новый Jupyter Notebook: **File → New → Notebook → Python 3 (ipykernel)**.

Сохраните новый Jupyter Notebook под именем `Pr_15_report.ipynb` (в `~/project/reports/Pr_15`)
...
1. Сохраните Jupyter Notebook `Pr_15_report.ipynb`
2. Загрузите файл в свой репозиторий в GitLab:
   - В левом окне Jupyter Lab (файловом менеджере) перейдите в каталог `project` (поднимитесь на два уровня вверх: из `Pr_15` в `reports` -> `project`)
   - Создайте новую вкладку типа `Terminal` (File -> New -> Terminal)
   - Введите

```bash
git add . && git commit -m "Pr_15 notebook report" && git push
```
