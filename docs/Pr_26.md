...
0. Подключитесь к Jupyter-Hub-ИИСТ-НПИ из [22й работы](Pr_22.md#%EF%B8%8F-настройка-среды)
...
1. Подготовьте отчёт в Markdown: `Pr_26_<группа>_<номер>.md` (или `reports/Pr_26/Pr_26_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные модели;
   - выбранные метрики классификации и понимания текста;
   - расчёты, примеры текста и предсказаний, а также их интерпретацию;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_26_<группа>_<номер>.md
