...
1. Подготовьте отчёт в Markdown: `Pr_28_<группа>_<номер>.md` (или `reports/Pr_28/Pr_28_<группа>_<номер>.md`).
2. В отчёте укажите:
   - загрузку данных и использованные модели;
   - выбранные метрики кластеризации, регрессии и оптимизации;
   - расчёты, графики и выводы по каждой метрике;
   - ответы на контрольные вопросы из раздела «Контрольные вопросы» ниже.
3. Загрузите отчёт в GitLab:

```bash
git add Pr_28_<группа>_<номер>.md
git commit -m "Pr_28 report"
git push
```
