...
for i in $(seq 1 30); do
    if docker exec nextcloud occ status 2>/dev/null | grep -q "installed"; then
...
docker exec nextcloud php occ config:app:set onlyoffice DocumentServerUrl \
  --value "http://onlyoffice:80/demos/" 2>/dev/null || \
